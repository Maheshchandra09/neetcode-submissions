...
class Solution {
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        Map<Integer,List<String>> emailGroups = new HashMap<>();
        for(Map.Entry<String,Integer> entry: emailToAcc.entrySet()){
            String email = entry.getKey();
            int accId = entry.getValue();
            int p = u.find(accId);
            emailGroups.putIfAbsent(p,new ArrayList<>());
            emailGroups.get(p).add(email);
        }
        List<List<String>> mergedAccounts = new ArrayList<>();
        for(Map.Entry<Integer,List<String>>entry : emailGroups.entrySet()){
